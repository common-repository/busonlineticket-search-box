jQuery(document).ready(function () {
	...
	function getValues() {
		editedTypeValue = document.getElementById('editedType').value;
	}

	// Call the function to retrieve values immediately
	getValues();
	
	// Event listener to toggle dropdown visibility
	document.addEventListener('click', function (event) {
		var dropdownContainers = document.querySelectorAll('.dropdown-container2');
	...
	document.querySelector('.txtFromLoc2').addEventListener('click', function (event) {
		event.stopPropagation(); // Prevent the click event from reaching the document listener
		toggleDropdown('.divSearchResultFrom2');
	});

	// Event listener to toggle "To" dropdown visibility on input click
	document.querySelector('.txtToLoc2').addEventListener('click', function (event) {
		event.stopPropagation(); // Prevent the click event from reaching the document listener
		toggleDropdown('.divSearchResultTo2');
	});
	...
	jQuery('select[name="editedType"]').change(function () {
		...
		jQuery('.txtFromLoc2').val('');
		jQuery('.txtToLoc2').val('');			
	});		
	...
	jQuery(function () {			
		...
		jQuery(".txtFromLoc2:visible").on("keyup", function () {
			textFromBoxFetch();
		});

		jQuery(".txtFromLoc2:visible").on("focus", function () {
			textFromBoxFetch();
		});

		jQuery(".txtFromLoc2:visible").on("click", function () {
			// jQuery(this).val("");
			textFromBoxFetch();
		});

		jQuery(".txtToLoc2:visible").on("keyup", function () {
			textToboxFetch();
		});

		jQuery(".txtToLoc2:visible").on("focus", function () {
			textToboxFetch();
		});

		jQuery(".txtToLoc2:visible").on("click", function () {
			jQuery(this).val("");
			textToboxFetch();
		});
	...
	function choseType(type) 
	{
		...
		{
			_ticketType = type;
			// jQuery(".txtFromLoc2:visible, .txtToLoc2:visible").val("");
		}
		...
		jQuery(".divSearchResultFrom2").hide();
	}
	...
	function fetchFrom(fromVal) 
	{
		...
		jQuery(".div-from-location2:visible .divSearchResultFrom2 ul").empty();
		jQuery(".div-from-location2:visible .divSearchResultFrom2 ul").html(_code);
		jQuery(".div-from-location2:visible .divSearchResultFrom2").show();
	...
	function fetchKtmFrom(fromVal) 
	{
		...
		jQuery(".div-from-location2:visible .li-select-from").on("click", function () {
			addtoText("from", this);
		});
	}
	...
	function fetchTo(from, toVal) 
	{
		...
		_code += '</ul></div><div class="clearfix"></div>';
		jQuery(".div-to-location2:visible .divSearchResultTo2 ul").empty();
		jQuery(".div-to-location2:visible .divSearchResultTo2 ul").html(_code);
		jQuery(".div-to-location2:visible .divSearchResultTo2").show();
	...
	function addtoText(type, elem) 
	{
		if (type == "from") 
		{
			jQuery(".txtFromLoc2:visible").val(jQuery(elem).text());
			jQuery(".divSearchResultFrom2 ul").empty();
			jQuery(".divSearchResultFrom2").hide();
		}

		else if (type == "to") 
		{
			jQuery(".txtToLoc2:visible").val(jQuery(elem).text());
			jQuery(".divSearchResultTo2 ul").empty();
			jQuery(".divSearchResultTo2").hide();
		}
	}        
	...
	{
		if (_ticketType == "bus")
			fetchFrom(jQuery(".txtFromLoc2:visible").val());

		else if (_ticketType == "train")
			fetchKtmFrom(jQuery(".txtFromLoc2:visible").val());

		else if (_ticketType == "ferry")
			fetchFerryFrom(jQuery(".txtFromLoc2:visible").val());

		jQuery(".txtToLoc2:visible").val("");
	}

	function textToboxFetch() 
	{
		if (_ticketType == "bus")
			fetchTo(jQuery(".txtFromLoc2:visible").val(), jQuery(".txtToLoc2:visible").val());

		else if (_ticketType == "train")
			fetchKtmTo(jQuery(".txtFromLoc2:visible").val(), jQuery(".txtToLoc2:visible").val());

		else if (_ticketType == "ferry")
			fetchFerryTo(jQuery(".txtFromLoc2:visible").val(), jQuery(".txtToLoc2:visible").val());
	}
});